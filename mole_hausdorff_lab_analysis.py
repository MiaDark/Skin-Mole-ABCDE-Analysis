"""
mole_hausdorff_lab_analysis.py

Full pipeline for skin mole analysis using CIE L*a*b* (Lab) color space:

1. Segments moles using L (lightness) channel + Otsu thresholding
2. Saves cropped & centered 300x300 segmented mole images
3. Computes Hausdorff dimension for whole mole and border
4. Extracts Lab statistics (mean, median, std, min, max) from:
...
# ========================
# 1. Mole Segmentation (Lab → L channel)
# ========================
def segment_mole_lab(image_path, save_path=None, visualize=False):
    """Segment mole using Lab color space and Otsu on L (lightness) channel."""
    ...
    if visualize:
        ...
        images = [image, L_channel, blurred, thresholded, mask, segmented]
...
def process_folders_for_hausdorff(
    ...
    for filename in tqdm(
        os.listdir(benign_dir), desc="Benign (Lab Segmentation)"
    ):
        if filename.lower().endswith((".jpg", ".jpeg", ".png")):
            src = os.path.join(benign_dir, filename)
            save = os.path.join(segmented_benign_dir, filename)
            mask, _ = segment_mole_lab(src, save_path=save, visualize=False)
            if mask is None:
                continue
    ...
    # Process malignant
    for filename in tqdm(
        os.listdir(malignant_dir), desc="Malignant (Lab Segmentation)"
    ):
        if filename.lower().endswith((".jpg", ".jpeg", ".png")):
    ...
    df.to_csv("hausdorff_dimensions_lab.csv", index=False)
    print("Hausdorff dimensions saved → hausdorff_dimensions_lab.csv")
    return df


# ========================
# 4. Lab Feature Extraction with Border Toggle
# ========================
def extract_lab_features(
    segmented_dir, label, border_only=False, border_thickness=8
):
    """
    Extract Lab statistics from segmented moles.
    ...
    for filename in tqdm(
        ...
        desc=f"Lab Features ({label} - {'Border' if border_only else 'Whole'})",
    ):
        ...
        valid_pixels = img_lab[valid_mask > 0]
        ...
        L, a, b = valid_pixels[:, 0], valid_pixels[:, 1], valid_pixels[:, 2]
...
# ========================
def statistical_comparison_lab(features_df):
    ...
    results_df.to_csv("lab_statistical_results.csv", index=False)

    print("\n=== Significant Lab Features (p < 0.05) ===")
    print(results_df[results_df["P-value"] < 0.05])
    print("\n=== All Lab Statistical Results ===")
    print(results_df)
...
if __name__ == "__main__":
    ...
    print(f"\n Mole Analysis Pipeline (Lab) – {region_name} ===\n")

    print("Segmenting moles and computing Hausdorff dimensions...")
    ...
    benign_feats = extract_lab_features(
        SEGMENTED_BENIGN_DIR,
        "benign",
        border_only=EXTRACT_BORDER_ONLY,
        border_thickness=BORDER_THICKNESS,
    )
    malignant_feats = extract_lab_features(
        SEGMENTED_MALIGNANT_DIR,
        "malignant",
        border_only=EXTRACT_BORDER_ONLY,
        border_thickness=BORDER_THICKNESS,
    ...
    if not all_features:
        ...
        feature_csv = f"mole_lab_features{suffix}.csv"
        df.to_csv(feature_csv, index=False)
        print(f"Features saved → {feature_csv} ({len(all_features)} samples)")

        print("\Statistical comparison (benign vs malignant)...")
        statistical_comparison_lab(df)

    print("Outputs:")
    print("   • hausdorff_dimensions_lab.csv")
    print(f"   • {feature_csv}")
    print("   • lab_statistical_results.csv")
    print(
